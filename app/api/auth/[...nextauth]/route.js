    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        try {
          ...
          console.error("Credentials Auth Error:", error.response?.data?.message);
  ...
  callbacks: {
    async signIn({ user, account }) {
      if (account.provider === "github") {
        try {
          ...
          console.error("OAuth sign-in error:", error.response?.data?.message || error.message);
          // To send error to the client, you can redirect to an error page
          // Or return false to stop the sign-in process
          return `/login?error=${encodeURIComponent(error.response?.data?.message || "OAuthError")}`;
        }
      }
      return true;
    },
    ...
    async jwt({ token, user, trigger, session }) {
      ...
      if (user) {
       ...
       if (trigger === "update") {
        console.log("DEBUG: JWT update triggered. Fetching fresh profile...");
        try {
          ...
          if (freshProfile) {
            ...
            console.error("DEBUG: Failed to fetch fresh profile during JWT update:", error);
